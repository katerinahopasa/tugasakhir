        ...
        <div class="card">
              ...
              @endif
                <div class="card-header">
                    <h6 class="text-center"><caption><b>DATA NERACA LAPORAN</b></caption></h6>
                  </div>
                  <div class="card-body">
                   <div class="panel-body">

              <!-- {!! Form::open(['url' => 'neraca-laporan/cari', 'method' => 'get', 'class' => 'form-inline text-center']) !!}
              <div class="col-md-3">
              ...
              {!! Form::close() !!} -->
                        ...
                        <tbody>
                          <?php $i = 1 ?>
                          @foreach($daftar_neracalaporan as $data)
                          <tr>
                            <td>{{ $i++ }}</td>
                            <td>{{ date('F Y', strtotime('1-'.$data->waktu)) }}</td>
                            <td>
                              <a href="{{ url('neraca-laporan/detail/'.date('Y-m-d', strtotime('1-'.$data->waktu))) }}" class="btn btn-info">
                                Detail
                              </a>
                              <a target="_BLANK" href="{{ url('cetak-neraca-laporan/'.date('Y-m-d', strtotime('1-'.$data->waktu))) }}" class="btn btn-outline-secondary"><i class="fa fa-print "></i> &nbsp; CETAK PDF</a>
                              <a target="_BLANK" href="{{ url('cetak-neraca-laporan/excel/'.date('Y-m-d', strtotime('1-'.$data->waktu))) }}" class="btn btn-outline-secondary"><i class="fas fa-file-excel"></i> &nbsp; CETAK EXCEL</a>
                            </td>
                          </tr>
                        @endforeach
                      </tbody>
