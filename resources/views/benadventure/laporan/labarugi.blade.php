       ...
       <div class="col-md-12">
        <div class="card">
              ...
              @endif
                    ...
                    <h6 class="text-center"><caption><b>DATA LAPORAN LABA RUGI</b></caption></h6>
      				    </div>
                  <div class="card-body">
      					   <div class="panel-body">

              <!-- {!! Form::open(['url' => 'labarugi/cari', 'method' => 'get', 'class' => 'form-inline text-center']) !!}
              <div class="col-md-3">
              ...
              {!! Form::close() !!} -->
                        ...
                        <tbody>
                          <?php $i = 1 ?>
                          @foreach($daftar_labarugi as $data)
                          <tr>
                            <td>{{ $i++ }}</td>
                            <td>{{ date('F Y', strtotime('1-'.$data->waktu)) }}</td>
                            <td>
                              <a href="{{ url('labarugi/detail/'.date('Y-m-d', strtotime('1-'.$data->waktu))) }}" class="btn btn-info">
                                Detail
                              </a>
                              <a target="_BLANK" href="{{ url('cetak-labarugi/'.date('Y-m-d', strtotime('1-'.$data->waktu))) }}" class="btn btn-outline-secondary"><i class="fa fa-print "></i> &nbsp; CETAK PDF</a>
                              <a target="_BLANK" href="{{ url('cetak-labarugi/excel/'.date('Y-m-d', strtotime('1-'.$data->waktu))) }}" class="btn btn-outline-secondary"><i class="fas fa-file-excel"></i> &nbsp; CETAK EXCEL</a>
                            </td>
                          </tr>
                        @endforeach
                      </tbody>
